...
def main(args):
    ...
    # test_views = [5,5,5,5]
    # #train_views = [i for i in range(2) if i not in test_views]
    # train_views = [i for i in range(6)]
    # test_views = [[6+i*20,11+i*20,16+i*20] for i in range(6)]
    # train_views = [[1+i*20, 1 if i == 5 else 21+i*20] for i in range(6)]
    train_views = [[0,4]]
    test_views = [[1,2,3]]
    index = {}
    for torch_file in tqdm(sorted(glob(os.path.join(data_dir, "*.torch")))):
        ...
        for scene_data in scene_datas:
            ...
            for seq_idx, (train_view, test_view) in enumerate(zip(train_views, test_views)):
                # cur_nn_index = nearest_fixed_views
                contexts = tuple(train_view)
                targets = tuple(test_view)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="VinAI")
